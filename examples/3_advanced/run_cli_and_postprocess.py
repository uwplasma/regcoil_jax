#!/usr/bin/env python3
"""Advanced end-to-end runner (CLI + optional figures/VTK).

Default input: examples/3_advanced/regcoil_in.lambda_search_1
"""
...
def main() -> None:
    default_input = Path(__file__).with_name("regcoil_in.lambda_search_1")
    ...
    print("[examples/3_advanced] running:", " ".join(cmd))
    ...
    print("[examples/3_advanced] postprocess:", " ".join(cmd))
    subprocess.run(cmd, cwd=str(project_root), check=True)
